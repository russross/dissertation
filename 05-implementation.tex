...
walk & Navigate directories and/or establish file handles \\
...
For basic file operations, an envoy acts like a 9p server to the client. When a file is owned remotely, the local envoy acts as a proxy server and forwards requests to the appropriate envoy. Like client connections, forwarded transactions are tracked relative to file handles owned by specific users.
...
* Separate service, same executable
...
... and how it's cached (with the next subsubsection?)

\subsubsection{Caching file handles}
