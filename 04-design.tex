...
\begin{itemize}
\item Read operations--easy!  + atime
\item readdir
\item Walk, including caching and race condition detection
\item Write operations--thaw + mtime synchronization
\item Rename, in all its glory, + walk invalidation on chmod or chown
\item Delete
\item Hard link and its complications (dead object in storage layer? special log file/hidden directory catalog?)\label{sec:hard-links}
\end{itemize}
